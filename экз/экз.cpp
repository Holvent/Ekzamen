...
bool Winn(int** p, int s) {
	...
	for (int i = 0; i < s; i++)
		for (int j = 0; j < s; j++) {
			...
			if (s == 4) {											//temp=0 будет присваиваться в зависимости от режима, если попытаться в одном условии сделать(как было), 
				if (temp == 16) temp = 0;							//то в режиме 4х4 он вместо 9 будет 0 ставить, хоть || хоть && ставь не робии, так что пусть будет так.
			}
...
void Game(int s, int choise_shuffle) {
	...
	do{
		...
		Move(pa, s, pi, pj,1);
	} while (key!=ESC&&Winn(pa, s));
	system("cls");
	if (key == ESC) Menu(2, 0,0);                       
	while (key != ESC) {
		Print(pa, s);
		deletearr(pa, s);
...
int main() {
	...
	do{
		Menu(2, 0);							//2 главное меню 4 для выбора режима 6 выбор мешанины
	} while (true);
}
